...
let activeThemeIndex = parseInt(localStorage.getItem("selectedTheme")) || 0;
...
/* ================= 2. GAME STATE ================= */
let currentScene = SCENES.PRELOAD;
let score = 0;
...
document.addEventListener("touchstart", (e) => { e.preventDefault(); handleInputStart(e); }, {passive: false});
...
// --- PLACE THIS AT THE START OF YOUR CLICK/TOUCH FUNCTION ---
...
if(isAdcardOpen){
  const adcardY = HEIGHT * 0.3;
          ...
          // Watch Ad
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.15, y: adcardY +170, width:250, height:45})) {
          playClickSound();
            alert("Loading Ad... (Earned 10 Coins!)");
            totalCoins += 10;
        ...
        // Close Button
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.825, y: adcardY + 6, width:40, height:40})) {
            isAdcardOpen = false;
            playClickSound();
            return;
        }
...
// --- SHOP OVERLAY CLICK DETECTION ---
if (isShopOpen) {
    const box = { x: WIDTH * 0.05, y: HEIGHT * 0.08, width: WIDTH * 0.9, height: HEIGHT * 0.84 };

    // 1. Back Button
    if (pos && isInside(pos.x, pos.y, { x: box.x + 15, y: box.y + 15, width: 75, height: 35 })) {
        isShopOpen = false;
        playClickSound();
        return;
    }

    // 2. Buy Magnet (Cost: 30)
    if (pos && isInside(pos.x, pos.y, { x: box.x + 120, y: box.y + 175, width: 70, height: 28 })) {
        buyItem("magnets", 30);
        playClickSound();
        return;
    }

    // 3. Buy Shield (Cost: 20)
    if (pos && isInside(pos.x, pos.y, { x: box.x + 25, y: box.y + 175, width: 70, height: 28 })) {
        buyItem("shields", 20);
        playClickSound();
        return;
    }
    
    if (pos && isInside(pos.x, pos.y, { x: box.x + 215, y: box.y + 175, width: 70, height: 28 })) {
        buyItem("ghosts", 50);
        playClickSound();
        return;
    }
            ...
            // Watch Ad
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.15, y: box.y+450, width:250, height:45})) {
          playClickSound();
            alert("Loading Ad... (Earned 10 Coins!)");
            totalCoins += 10;
            localStorage.setItem("totalCoins" , totalCoins);
    ...
    THEMES.forEach((t, i) => {
      ...
      let btnY = cardY + cardHeight + 6; 
        ...
        if (pos && isInside(pos.x, pos.y, { x: cardX + 5, y: btnY, width: themeWidth - 10, height: 28 })) {
          playClickSound();
            if (t.owned) {
                activeThemeIndex = i;
                applyTheme(i);
    ...
    return; // Stop input from leaking into underlying scenes
}
// 1. Check Pause Button (while in game)
if (currentScene === SCENES.GAME && !isPaused) {
    if (pos && isInside(pos.x, pos.y, { x: 15, y: 15, width: 40, height: 40 })) {
        isPaused = true;
        playClickSound();
...
// 2. Check Pause Menu Clicks
if (isPaused) {
    const boxY = HEIGHT * 0.2;
    
    // CONTINUE Button
    if (pos && isInside(pos.x, pos.y, {x: WIDTH/2 - 70, y: boxY + 210, width: 140, height: 45})) {
        isPaused = false;
        playClickSound();
        updateBackgroundMusic();
        return;
    ...
    if (pos && isInside(pos.x, pos.y, {x: WIDTH/2 - 70, y: boxY + 270, width: 140, height: 45})) {
        isPaused = false;
        playClickSound();
        currentScene = SCENES.START;
        return;
    }
    return; // Block other game actions (like jumping)
}
...
if (isSettingsOpen) {
  const menuY = HEIGHT * 0.3;
        // Sound Toggle
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.575, y: menuY+50, width:60, height:30})) {
            isSoundOn = !isSoundOn;
            playClickSound();
            return;
        }
        ...
        // Music toggle
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.575, y: menuY+110, width:60, height:30})) {
            isMusicOn = !isMusicOn;
            playClickSound();
            updateBackgroundMusic();
            return;
        ...
        // Watch Ad
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.15, y: menuY+170, width:250, height:45})) {
          playClickSound();
            alert("Loading Ad... (Earned 10 Coins!)");
            totalCoins += 10;
            localStorage.setItem("totalCoins" , totalCoins);
        ...
        // Close Button
        if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.825, y: menuY+ 6, width:40, height:40})) {
            isSettingsOpen = false;
            playClickSound();
            return;
        }
        return; // Block other game clicks while menu is open
    }

    // IF START SCENE, CHECK FOR SETTINGS GEAR
    if (currentScene === SCENES.START) {
      isDead = false;
      isPaused = false;
        if (pos && isInside(pos.x, pos.y, { x: 10, y: 10, width: 40, height: 40 })) {
            isSettingsOpen = true;
            playClickSound();
            return;
        }
...
if (currentScene === SCENES.GAME) {
    // Define the same areas where you drew your buttons
    const magnetbtn = { x: 10, y: HEIGHT - 140, width: 60, height: 60 };
    const shieldbtn = { x: 10, y: HEIGHT - 70, width: 60, height: 60 };
    const ghostbtn = {x: 10, y:HEIGHT-210, width: 60, height : 60};
    
    

    // 1. Check Magnet Button
    if (pos && isInside(pos.x, pos.y, { x: 10, y: HEIGHT-140,width:60, height:60 })) {
        playPowerUpSound();
        if (inventory.magnets > 0 && !isMagnetActive) {
            inventory.magnets--;
            isMagnetActive = true;
            magnetTimer = 1500; // Activation time
            // Save the new count
    ...
    // 2. Check Shield Button
    if (pos && isInside(pos.x, pos.y, { x: 10, y: HEIGHT-70, width:60, height:60 })) {
        playPowerUpSound();
    ...
    //3.Ghost button
    if (pos && isInside(pos.x, pos.y, { x: 10, y: HEIGHT-210, width:60, height:60 })) {
      playPowerUpSound();
...
if (currentScene === SCENES.START){
  isDead = false;
  isPaused = false;
    if (pos && isInside(pos.x, pos.y, { x: WIDTH/2-150, y: HEIGHT*0.70, width:130, height:55 })) {
      isShopOpen = true;
      playClickSound();
    return;
    }
...
if (currentScene === SCENES.START) {
  isDead = false;
  isPaused = false;
    if (pos && isInside(pos.x, pos.y, { x: WIDTH * 0.55, y: HEIGHT*0.70, width:130, height:55 })) setScene(SCENES.INSTRUCTIONS);
    playClickSound();
    return;
  }
  ...
  if (currentScene === SCENES.GAMEOVER) {
    ...
    // 1. RESTART
    if (pos &&isInside(pos.x, pos.y, {x: WIDTH/2 - 140, y: boxY + 80, width: 140, height: 45})) {
      playClickSound();
        setScene(SCENES.GAME);
    }

    // 2. REVIVE (Watch Ad)
    if (pos && isInside(pos.x, pos.y, {x: WIDTH/2 - 140, y: boxY + 20, width: 140, height: 45})) {
      playClickSound();
        alert("Ad Loading... Player Revived!");
        isDead = false; 
    ...
    // 3. DOUBLE COINS
    if (pos && isInside(pos.x, pos.y, {x: WIDTH * 0.575, y: boxY + 140, width: 140, height: 45})) {
      playClickSound();
        alert("Ad Loading... Coins Doubled!");
        // We only double the coins earned in THIS specific place run
        // (Assuming you track 'sessionCoins') and 
    ...
    // 4. HOME
    if (pos && isInside(pos.x, pos.y, {x: WIDTH/2 - 140, y: boxY + 140, width: 140, height: 45})) {
      playClickSound();
        currentScene = SCENES.START;
        isDead = false;
    }
...
function save() {
    localStorage.setItem("totalCoins", totalCoins);
    localStorage.setItem("inventory", JSON.stringify(inventory));
    localStorage.setItem("selectedTheme", activeThemeIndex);
}
...
function updateBackgroundMusic() {
  // Play music if music is ON AND:
  // 1. We are NOT in the active GAME scene (Start, Shop, GameOver, etc.), OR
  // 2. We ARE in the GAME scene, but the game is PAUSED
  const shouldPlayMusic = isMusicOn && (currentScene !== SCENES.GAME || isPaused);
...
function triggerDeath() {
  ...
  playDeathSound();
  shakeIntensity = 10;
  isDead = true; currentScene = SCENES.GAMEOVER;
...
// Preload & Start Screens (Shortened)
/* ================= 8. SYSTEM / UTILS (Updated drawPreload) ================= */
function drawPreload() {
    preloadTime++;
    
    // Smooth floating/hovering animation using a sine wave
    const logoFloat = Math.sin(preloadTime * 0.08) * 12;
    
    // Infinite Phase Switching (toggles smoothly every ~1.5 seconds)
    const currentPreloadPhase = Math.sin(preloadTime * 0.08) > 0 ? "A" : "B";
    const avatarColor = currentPreloadPhase === "A" ? PHASE_A_COLOR : PHASE_B_COLOR;
    const avatarOutline = currentPreloadPhase === "A" ? PHASE_B_COLOR : PHASE_A_COLOR;

    ctx.save();
    // Center of canvas + floating vertical offset
    ctx.translate(WIDTH / 2, HEIGHT / 2 - 40 + logoFloat);

    // 1. Dual-Phase Rotating Portal Rings
    ctx.lineWidth = 4;
    
    // Outer Phase A Ring
    ctx.strokeStyle = PHASE_A_COLOR;
    ctx.beginPath();
    ctx.arc(0, 0, 75, preloadTime * 0.025, preloadTime * 0.025 + Math.PI * 1.2);
    ctx.stroke();

    // Inner Phase B Ring (Counter-rotating)
    ctx.strokeStyle = PHASE_B_COLOR;
    ctx.beginPath();
    ctx.arc(0, 0, 88, -preloadTime * 0.025, -preloadTime * 0.025 + Math.PI * 1.2);
    ctx.stroke();

    // 2. Hovering Large Player Avatar (80x80)
    const pSize = 80;
    const half = pSize / 2;
    ...
    if (currentPreloadPhase === "B") { // Open O-Mouth
      ctx.beginPath();
    ...
    // Progress Bar Fill
    ctx.fillStyle = PHASE_B_COLOR; 
    ctx.fillRect(barX, barY, (preloadTime / 90) * barW, 6);

    // Transition to Start Scene once loaded
    if (preloadTime > 90) { 
      preloadTime = 0; 
      currentScene = SCENES.START; 
      titleX = -400; 
...
function gameLoop() {
  ...
  if (shakeIntensity > 0) {
        ...
        ctx.translate(dx, dy); // Shift the whole canvas
        shakeIntensity *= 0.9; // Gradually stop shaking
  ...
  // Render Dynamic Theme-Matching City Background
  drawCityBackground();

  
  if (currentScene === SCENES.PRELOAD) drawPreload();
  
  else if (currentScene === SCENES.START) drawStart();
  
  else if (currentScene === SCENES.INSTRUCTIONS) drawInstructions();
  
  else {
    if (currentScene === SCENES.GAME && !isDead) {updatePlayer(); updateWalls(); updateCoins(); updatePowerUps(); }
    
    drawPlayer();
    drawWalls(); drawCoins(); drawPowerUps();
    ...
    // Spawn 1 particle every frame
if (Math.random() > 0.4) {
    particles.push({
        x: player.x,
        y: player.y + player.size / 2,
        size: Math.random() * 6 + 2,
        vx: -Math.random() * 2 - 2, // Move left (backwards)
        vy: (Math.random() - 0.5) * 2, // Slight up/down drift
        life: 1.0, // Opacity starts at 1
