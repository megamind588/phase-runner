  ...
  if (pos) {
    if (isAdcardOpen) {
      ...
      if (isInside(pos.x, pos.y, {x: WIDTH * 0.15, y: adcardY + 170, width: 250, height: 45})) {
        ...
        setTimeout(() => { isAdcardOpen = false; }, 500); 
        return;
      }
    ...
    if (isShopOpen) {
      ...
      if (isInside(pos.x, pos.y, {x: WIDTH * 0.15, y: box.y + 450, width: 250, height: 45})) {
        ...
        totalCoins += 10;
        localStorage.setItem("totalCoins", totalCoins);
        setTimeout(() => { isShopOpen = false; }, 500); 
        return;
      }
    ...
    if (isSettingsOpen) {
      ...
      if (isInside(pos.x, pos.y, {x: WIDTH * 0.15, y: menuY + 170, width: 250, height: 45})) {
        ...
        totalCoins += 10;
        localStorage.setItem("totalCoins", totalCoins);
        setTimeout(() => { isSettingsOpen = false; }, 500); 
        return;
      }
...
function gameLoop(timestamp) {
  // Always request next frame at top of loop
  requestAnimationFrame(gameLoop);

  // Fallback timestamp if missing, or reset on epoch mismatch
  if (!timestamp) timestamp = performance.now();
  if (!lastTime || timestamp < lastTime) lastTime = timestamp;

  const deltaTime = timestamp - lastTime;

  // Handle tab switching / long lag spikes
  if (deltaTime > 1000) {
    lastTime = timestamp;
    return;
  }

  // Only render & update when 16.67ms has passed (60 FPS cap)
  if (deltaTime >= FRAME_TIME) {
    ...
    if (currentScene === SCENES.PRELOAD) drawPreload();
      ...
      if (currentScene === SCENES.GAME && !isDead) {
        updatePlayer(); 
        updateWalls(); 
        updateCoins(); 
        updatePowerUps(); 
